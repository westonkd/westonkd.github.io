...
(function () {
  ...
  var fromTable$1 = function (table) {
    ...
    var elems = __spreadArray(
      __spreadArray([], columnGroups$1, true),
      rows,
      true
    );
          ...
          if (eq$1(startTable, finishTable)) {
            ...
            return ancestors(start, finish).shared.bind(function (lca) {
              return closest$1(lca, "table", isRoot).bind(function (lcaTable) {
                var finishAncestorCells = ancestors$3(
                  finish,
                  "td,th",
                  getIsRoot(lcaTable)
                );
                var finishCell =
                  finishAncestorCells.length > 0
                    ...
                    : finish;
                var startAncestorCells = ancestors$3(
                  start,
                  "td,th",
                  getIsRoot(lcaTable)
                );
  ...
  var getWidthFrom = function (warehouse, table, getWidth, fallback) {
    ...
    var columns$1 = Warehouse.hasColumns(warehouse)
      ? justCols(warehouse)
      : columnCells;
  ...
  var adjustAndRedistributeWidths$1 = function (
    _table,
    list,
    details,
    tableSize,
  ...
  var determineDefaultStyles = function (editor) {
    ...
    if (isPixelsForced(editor)) {
      var dom = editor.dom;
      var parentBlock =
        (_a = dom.getParent(editor.selection.getStart(), dom.isBlock)) !==
          null && _a !== void 0
          ? _a
          : editor.getBody();
  ...
  var replace = function (cell, comparator, substitute) {
    return elementnew(
      substitute(cell.element, comparator),
      true,
      cell.isLocked
    );
  };
  ...
  var modification = function (generators, toData) {
    ...
    var nuRow = function (data) {
      return isColgroup(data.element)
  ...
  var opMakeCellsHeader = function (
    initialGrid,
    details,
    comparator,
    genWrappers
  ) {
    var newGrid = replaceCells(
      initialGrid,
      details,
      comparator,
      genWrappers.replaceOrInit
  ...
  var opUnmakeCellsHeader = function (
    initialGrid,
    details,
    comparator,
    genWrappers
  ...
  var adjustAndRedistributeWidths = function (
    ...
    return adjustAndRedistributeWidths$1(
      table,
      list,
      details,
      behaviours.sizing,
  ...
  var TableActions = function (editor, cellSelection, lazyWire) {
    ...
    var execute = function (operation, guard, mutate, lazyWire, effect) {
      return function (table, target, noEvents) {
        ...
        return guard(table)
          ? operation(wire, table, target, generators, behaviours).bind(
              function (result) {
    ...
    var unmakeCellsHeader$1 = execute(
      unmakeCellsHeader,
      always,
      noop,
      lazyWire,
  ...
  var setClipboardElements = function (setClipboard) {
    return function (elems) {
      var elmsOpt =
        elems.length > 0 ? Optional.some(fromDom(elems)) : Optional.none();
  ...
  var copyCols = function (table, target) {
    ...
    return details.map(function (selectedCells) {
      ...
      return __spreadArray(
        __spreadArray([], fakeColGroups, true),
        fakeRows,
        true
      );
    });
  ...
  var filterNoneItem = function (list) {
    return bind$2(list, function (item) {
      if (isListGroup(item)) {
        return [
          __assign(__assign({}, item), { menu: filterNoneItem(item.menu) }),
        ];
      } else {
  ...
  var generateMenuItemsCallback = function (
    editor,
    selections,
    items,
    format,
    ...
    return function (callback) {
      return callback(
        buildMenuItems(editor, selections, items, format, onAction)
      );
    };
  };
  ...
  var extractAdvancedStyles = function (dom, elm) {
    ...
    return {
      ...
      bordercolor: getRaw$2(element, "border-color")
        .map(rgbToHex(dom))
        .getOr(""),
      backgroundcolor: getRaw$2(element, "background-color")
        .map(rgbToHex(dom))
        .getOr(""),
    };
  };
    ...
    var extractAdvancedStyleData = function (dom) {
      return {
        ...
        backgroundcolor: rgbToHex(dom)(
          get$c(style, "background-color").getOr("")
        ),
      };
    };
    var defaultData = {
  ...
  var getSelectionStartCellOrCaption = function (editor) {
    return getSelectionCellOrCaption(
      getSelectionStart(editor),
      getIsRoot(editor)
    );
  };
  ...
  var registerCommands = function (
    ...
    var toggleTableCellClass = function (_ui, clazz) {
      performActionOnSelection(function (table) {
        ...
        var formatterAction = allHaveClass
    ...
    each$1(
      {
        ...
        mceTableCellType: actOnType(function (type) {
          return type === "th"
            ? actions.makeCellsHeader
            : actions.unmakeCellsHeader;
        }),
  ...
  var handle$1 = function (event, editor, cellSelection) {
    if (event.keyCode === global$1.TAB) {
      ...
      var container = SugarElement.fromDom(
        event.shiftKey ? rng.startContainer : rng.endContainer
      );
        ...
        findCell(event.target, isRoot).each(function (finish) {
          identify(start, finish, isRoot).each(function (cellSel) {
            ...
            if (boxes.length === 1) {
              ...
              var isCellClosestContentEditable = is(
  ...
  var hasInternalTarget = function (e) {
    return (
      has(SugarElement.fromDom(e.target), "ephox-snooker-resizer-bar") === false
    );
  };
  ...
  var getSelectionTargets = function (editor, selections) {
      ...
      return getStart().bind(function (startCellOrCaption) {
        return flatten(
          lift2(
            ...
            getEnd().bind(table),
            function (startTable, endTable) {
              if (eq$1(startTable, endTable)) {
                if (isCaption(startCellOrCaption)) {
                  return Optional.some(noMenu(startCellOrCaption));
                } else {
                  return Optional.some(
                    forMenu(selections, startTable, startCellOrCaption)
                  );
                }
              }
              return Optional.none();
    ...
    var onSetupTableRowHeaders = onSetupTableHeaders(
      "mceTableRowType",
      "header"
    );
    var onSetupTableColumnHeaders = onSetupTableHeaders(
      "mceTableColType",
      "th"
    );
    ...
    editor.ui.registry.addContextMenu("table", {
      update: function () {
        ...
        return selectionTargets
          .targets()
          .fold(constant(""), function (targets) {
